...
const ContentItem = ({ type, title, path }) => {
  ...
  const handleSave = () => {
    ...
    setIsEditing(false);
    // Здесь можно добавить отправку на сервер, если нужно сохранять навсегда
  };
    ...
    <div className="bg-white shadow rounded-xl p-4 border hover:shadow-md transition-all whitespace-pre-wrap">
      ...
      {type === 'picture' ? (
        ...
        <a
          href={path}
          download
          className="text-sm mt-1 px-3 py-1 inline-block rounded bg-blue-50 text-blue-600 hover:bg-blue-100 transition"
        >
          Скачать
        </a>
      ) : type === 'text' ? (
